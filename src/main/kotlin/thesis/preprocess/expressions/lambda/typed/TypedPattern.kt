...
sealed class TypedPattern<T: Implication<T>> : Pattern, Typed<Parametrised<T>> {
    ...
    abstract fun <S: Implication<S>> modifyType(action: (T) -> S): TypedPattern<S>
    ...
    data class Variable<T: Implication<T>>(
        ...
        override fun <S : Implication<S>> modifyType(action: (T) -> S) = Variable(
                pattern,
    ...
    data class Object<T: Implication<T>>(
        ...
        override fun <S : Implication<S>> modifyType(action: (T) -> S) = Object(
                pattern,
                type.modifyType(action)
        )
    ...
    data class Constructor<T: Implication<T>>(
        ...
        override fun <S : Implication<S>> modifyType(action: (T) -> S) = Constructor(
                name,
